...
using Timereporting.Infrastructure.Data.Repositories;

namespace Timereporting.Infrastructure.Data.Services
{
    public class AppDbContextSeedingService : IHostedService
    {
        private readonly IServiceProvider _services;
        private readonly ILogger<AppDbContextSeedingService> _logger;
        private CancellationTokenSource? _cancellationTokenSource;

        public AppDbContextSeedingService(IServiceProvider services, ILogger<AppDbContextSeedingService> logger)
        {
            _services = services;
            _logger = logger;
        }
            ...
            try
            {
                ...
                while (currentRetry < maxRetries)
                {
                    try
                    {
                        ...
                        await UserSeedingRepository.SeedDataAsync(context);
                        await ReportSeedingRepository.SeedDataAsync(context);
                        await WorkplaceSeedingRepository.SeedDataAsync(context);
